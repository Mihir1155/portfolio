import React, { useState, useEffect } from "react";
import heroImg from "../../assets/images/hero3.png";
import CountUp from "react-countup";
import { Circle } from "rc-progress";
import video from "../../assets/images/port.mp4";
import { Typewriter } from "react-simple-typewriter";

const Hero = () => {
  const [count, setCount] = useState(1);

  // useEffect(() => {
  //   if (count > 0) {
  //     setTimeout(() => setCount((prev) => prev.show[0] + 1), 1000);
  //   }
  // }, [count]);


  return (
    <section className="pt-0" id="about">
      <div className="container pt-14">
        <div className="md:flex items-center justify-between sm:flex-col md:flex-row">
          {/* -----------hero left content--------- */}
          <div className="w-full md:basis-1/2">
            <h5
              data-aos="fade-right"
              data-aos-duration="1500"
              className="text-headingColor font-[500] text-[16px]"
            >
              Hello Welcome
            </h5>
            <h1
              data-aos="fade-up"
              data-aos-duration="1500"
              className="text-headingColor font-[800] text-[1.8rem] sm:text-[40px] leading-[35px] sm:leading-[46px] mt-5"
            >
              I'm Mihir Sahoo
              <br />
              <Typewriter
               words={['Web Developer','Content Creator', 'Professional Coder',]}
               loop={1000}
               cursor
               cursorStyle='.'
               typeSpeed={70}
               deleteSpeed={50}
               delaySpeed={1000}
              //  onLoopDone={handleDone}
              //  onType={handleType}
              />
            </h1>

            <div
              data-aos="fade-up"
              data-aos-duration="1800"
              data-aos-delay="200"
              className="flex items-center gap-7 mt-7"
            >
              <a href="#contact">
                <button
                  className="bg-primaryColor font-[500] flex items-center gap-2 
                        hover:bg-smallTextColor ease-in duration-300 py-2 px-4 rounded-[8px]"
                >
                  <i class="ri-mail-fill"> Hire me</i>
                </button>
              </a>
              <a
                href={require("../../assets/images/ResumeM.pdf")}
                className="text-smallTextColor font-[600] text-[16px] border-b border-solid"
                download="Mihir_Resume"
              >
                Resume
              </a>
            </div>
            <p
              data-aos="fade-left"
              data-aos-duration="1500"
              className="flex gap-2 text-headingColor mt-12 font-[500] pl-[0.2rem] text-[15px] leading-7 sm:pl-1 sm:pr-10"
            >
              <span>
                <i class="ri-apps-2-line"></i>
              </span>
              I am a web developer with a passion for creating beautiful and
              functional websites. I have experience in building websites using
              HTML, CSS, JavaScript, and various frameworks and libraries, such
              as React, Tailwind Css, and Three js. I also have knowledge of web
              design principles, user interface design, and responsive web
              development.
            </p>
            <div className="flex items-center gap-9 mt-14">
              <span className="text-smallTextColor text-[20px] font-[600]">
                Follow Me:
              </span>
              <span >
                <a
                  target="#"
                  href="https://www.linkedin.com/in/mihir-sahoo-483354247"
                  className="text-smallTextColor text-[15px] font-[600]"
                >
                  <i class="ri-linkedin-fill"></i>
                </a>
              </span>
              <span>
                <a
                  target="#"
                  href="https://instagram.com/mihir._.sahoo?igshid=NGExMmI2YTkyZg=="
                  className="text-smallTextColor text-[15px] font-[600]"
                >
                  <i class="ri-instagram-fill"></i>
                </a>
              </span>
              <span>
                <a
                  target="#"
                  href="https://twitter.com/mihirsahoo1155?t=jSiLGSuBBmnEDox5hQ7wRg&s=08"
                  className="text-smallTextColor text-[15px] font-[600]"
                >
                  <i class="ri-twitter-fill"></i>
                </a>
              </span>
              <span>
                <a
                  target="#"
                  href="https://github.com/Mihir1155"
                  className="text-smallTextColor text-[15px] font-[600]"
                >
                  <i class="ri-github-fill"></i>
                </a>
              </span>
            </div>
          </div>
          {/* -------------hero left content end----------------- */}
          {/* ---------hero img------------- */}
          <div className="basis-1/3 mt-10 sm:mt-0">
            <figure className="flex items-center justify-center">
              <img src={heroImg} alt="" className="h-[250px]"/>
            </figure>
          </div>
          {/* ------------hero img end----------- */}
          {/* ---------hero content right---------- */}
          {/* <div
            className="md:basis-1/5 justify-between text-center mt-10 flex-wrapg3
          md:mt-0 md:flex-col md:justify-end md:text-end "
          >
            <div className="mb-10">
              <h2 className="text-headingColor font-[600] text-[32px]">
                <CountUp start={0} end={50} duration={2} suffix="+" />
              </h2>
              <h4 className="text-headingColor font-[600] text-[32px]">
                Years of experience
              </h4> */}
              {/* <Circle
                  percent={count}
                  strokeWidth={3}
                  trailWidth={3}
                  className="h-[125px] w-[150px]"
                  /> */}
            {/* </div>
            <div className="mb-10">
              <h2 className="text-headingColor font-[600] text-[32px]">
                <CountUp start={0} end={100} duration={2} suffix="%" />
              </h2>
              <h4 className="text-headingColor font-[600] text-[32px]">
                Sucess rate
              </h4>
            </div>
            <div className="mb-10">
              <h2 className="text-headingColor font-[600] text-[32px]">
                <CountUp start={0} end={150} duration={2} suffix="+" />
              </h2>
              <h4 className="text-headingColor font-[600] text-[32px]">
                Happy clients
              </h4>
            </div>
            <div className="mb-10">
              <h2 className="text-headingColor font-[600] text-[32px]">
                <CountUp start={0} end={10} duration={2} suffix="+" />
              </h2>
              <h4 className="text-headingColor font-[600] text-[32px]">
                Projects completed
              </h4>
            </div>
          </div> */}
          {/* -----------hero content right end----------- */}
        </div>
      </div>
    </section>
  );
};

export default Hero;
